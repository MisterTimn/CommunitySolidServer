...
import { toRetryOnBoolean, retryFunctionUntil } from '../LockUtils';
 ...
 *
 * This allows us to reuse the AttemptSettings semantics similar to the {@link FileSystemResourceLocker} class.
 *
...
export class RedisLocker implements ReadWriteLocker, ResourceLocker, Finalizable {
   ...
   * to something else than `false` or rejects with an error.
   * @param fn - The function reference to retry until successful or stopped.
   */
  private async retryUntilFalse<T>(fn: () => Promise<RedisAnswer>): Promise<T> {
    return await retryFunctionUntil<any>(
      toRetryOnBoolean(false, (): any => fromResp2ToBool(fn())), this.attemptSettings,
    );
  }
  ...
  public async withReadLock<T>(identifier: ResourceIdentifier, whileLocked: () => (Promise<T> | T)): Promise<T> {
    ...
    await this.retryUntilFalse((): any => this.redisRw.acquireReadLock(key));
    try {
      return await whileLocked();
    } finally {
      await this.retryUntilFalse((): any => this.redisRw.releaseReadLock(key));
    }
  }

  public async withWriteLock<T>(identifier: ResourceIdentifier, whileLocked: () => (Promise<T> | T)): Promise<T> {
    const key = this.getReadWriteKey(identifier);
    await this.retryUntilFalse((): any => this.redisRw.acquireWriteLock(key));
    try {
      return await whileLocked();
    } finally {
      await this.retryUntilFalse((): any => this.redisRw.releaseWriteLock(key));
    }
  }
  ...
  public async acquire(identifier: ResourceIdentifier): Promise<void> {
    ...
    await this.retryUntilFalse((): any => this.redisLock.acquireLock(key));
  }

  public async release(identifier: ResourceIdentifier): Promise<void> {
    const key = this.getResourceKey(identifier);
    await this.retryUntilFalse((): any => this.redisLock.releaseLock(key));
  }
