...
export class FileDataAccessor implements DataAccessor {
  ...
  protected async writeMetadata(link: ResourceLink, metadata: RepresentationMetadata): Promise<boolean> {
    ...
    // When writing metadata for a document, do not remove the contentType when dealing with an unsupported media type.
    // (unsupported media type: when link.contentType does not match metadata.contentType)
    // This allows restoring the appropriate content type on data read (see getFileMetadata).
    const isUnsupportedMediaTypeDocument = !isContainerPath(link.filePath) && link.contentType !== metadata.contentType;
    if (!isUnsupportedMediaTypeDocument) {
