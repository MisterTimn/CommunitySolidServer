...
describe('A server with a file backend storage', (): void => {
  ...
  it('supports content types for which no extension mapping can be found (and falls back to application/octet-stream).',
    async(): Promise<void> => {
      ...
      const check2 = await pathExists(`${rootFilePath}/test$.bin`);
      expect(check2).toBe(true);
    });
});
